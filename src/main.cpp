...
static void (*handler[LASTEvent])(XEvent *) {
    ...
    /*FocusIn           */ focus,
    /*FocusOut          */ focus,
    /*KeymapNotify      */ nullptr,
    /*Expose            */ expose,
    /*GraphicsExpose    */ nullptr,
    /*NoExpose          */ nullptr,
    /*VisibilityNotify  */ visibility,
    /*CreateNotify      */ nullptr,
    /*DestroyNotify     */ nullptr,
    /*UnmapNotify       */ unmap,
    /*MapNotify         */ nullptr,
    /*MapRequest        */ nullptr,
    /*ReparentNotify    */ nullptr,
    /*ConfigureNotify   */ resize,
    /*ConfigureRequest  */ nullptr,
    /*GravityNotify     */ nullptr,
    /*ResizeRequest     */ nullptr,
    /*CirculateNotify   */ nullptr,
    /*CirculateRequest  */ nullptr,
    /*PropertyNotify    */ nullptr,
    /*SelectionClear    */ selclear,
    /*SelectionRequest  */ selrequest,
    /*SelectionNotify   */ selnotify,
    /*ColormapNotify    */ nullptr,
    /*ClientMessage     */ cmessage,
    /*MappingNotify     */ nullptr,
...
execsh(void) {
    ...
    if(opt_cmd)
        ...
        prog = sh;
    char* execparam[] = {prog, NULL};
    args = (opt_cmd) ? opt_cmd : execparam ;
...
ttynew(void) {
    int m, s;
    struct winsize w = {term.row, term.col, (short unsigned int)0, (short unsigned int)0};
...
tresize(int col, int row) {
    ...
    term.line  = (Glyph_**) xrealloc(term.line, row * sizeof(Line));
    term.alt   = (Glyph_**) xrealloc(term.alt,  row * sizeof(Line));
    term.dirty = (bool*   ) xrealloc(term.dirty, row * sizeof(*term.dirty));
    term.tabs  = (bool*   ) xrealloc(term.tabs, col * sizeof(*term.tabs));

    /* resize each row to new width, zero-pad if needed */
    for(i = 0; i < minrow; i++) {
        term.line[i] = (Glyph_*) xrealloc(term.line[i], col * sizeof(Glyph));
        term.alt[i]  = (Glyph_*) xrealloc(term.alt[i],  col * sizeof(Glyph));
    }

    /* allocate any new rows */
    for(/* i == minrow */; i < row; i++) {
        term.line[i] = (Glyph_*) xmalloc(col * sizeof(Glyph));
        term.alt [i] = (Glyph_*) xmalloc(col * sizeof(Glyph));
    }
    if(col > term.col) {
...
void
xhints(void) {
    XClassHint xclass = {opt_class ? opt_class : termname, termname};
...
xinit(void) {
    ...
    XRecolorCursor(xw.dpy, cursor, &c1, &c2);
...
void
xresettitle(void) {
    xsettitle(opt_title ? opt_title : "lute");
}
...
drawregion(int x1, int y1, int x2, int y2) {
    ...
    for(y = y1; y < y2; y++) {
        ...
        ic = ib = ox = 0;
        for(x = x1; x < x2; x++) {
            new_glyph = term.line[y][x];
            if(new_glyph.mode == ATTR_WDUMMY)
                continue;
            if(ena_sel && selected(x, y))
                new_glyph.mode ^= ATTR_REVERSE;
            if(ib > 0 && (ATTRCMP(base, new_glyph)
            ...
            sl = utf8decode(new_glyph.c, &unicodep, UTF_SIZ);
            memcpy(buf+ib, new_glyph.c, sl);
            ib += sl;
            ic += (new_glyph.mode & ATTR_WIDE)? 2 : 1;
        }
        if(ib > 0)
            xdraws(buf, base, ox, y, ic, ib);
    }
...
main(int argc, char *argv[]) {
    ...
    //TODO remove
    for (int i = 16; i < sizeof(colorname)/sizeof(char*);i++) {
        colorname[i] = 0;
    }
    colorname[256] = "#cccccc";
